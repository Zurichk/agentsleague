...
try:
    # Configurar path absoluto primero
    import sys
    import os
...
def get_agents_metrics():
    ...
    print(
        f"🔍 API métricas llamada - get_metrics_collector disponible: {get_metrics_collector is not None}")
    ...
    if get_metrics_collector:
        ...
        # Si no hay métricas reales, devolver estructura vacía
        if not all_metrics:
            return jsonify({
                'metrics': {
                    'overall': {
...
@app.route('/api/study-plans/<student_or_plan_id>')
...
def activate_email_reminders(plan_id):
    ...
    try:
        ...
        logger.error(f"Error activando avisos por email: {e}")
        return jsonify({'success': False, 'message': str(e)}), 500
